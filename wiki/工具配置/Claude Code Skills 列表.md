...
> 最后更新：2026-06-10 | 共 134 个 skills
...
### codegraph 📌 新增
- **功能：** 代码关系图分析 — 依赖关系、调用链、模块结构、循环依赖检测、架构层次、变更影响分析
- **触发：** "依赖关系", "调用链", "谁调用了", "被谁引用", "模块结构", "循环依赖", "依赖图", "调用图", "架构图", "模块耦合", "代码关系"
- **与 codereview（找 bug）+ code-simplifier（优化质量）形成三角互补，关注代码之间的连接关系和架构拓扑
...
### claudemem 📌 新增
- **功能：** 管理 Claude 持久化记忆系统，跨会话存储用户偏好、反馈、项目上下文和外部参考
- **触发：** "记住"、"记下来"、"保存这个"、"别忘了"、用户提出修改工作方式的反馈、分享个人偏好时
- **规则：** 写前检查重复、更新优于新建、删除错误记忆、链接相关记忆形成知识图谱
- **记忆位置：** `C:\Users\20159\.claude\projects\C--Users-20159-OneDrive-Desktop\memory\`
